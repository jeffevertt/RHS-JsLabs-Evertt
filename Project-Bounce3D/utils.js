...
function worldTo2D(pos) {
    let toScreenSpace = mat4x4.multiply(transProj, transCamera);
    let pos2D = toScreenSpace.transformPoint(pos);
    return pos2D;
}

function isInsideWorld(pos) {
    let buffer = ballRadius * 0.5;
    return ((pos.x < -worldBoxHalfSize.x - buffer) || (pos.x > worldBoxHalfSize.x + buffer) ||
            (pos.y < -worldBoxHalfSize.y - buffer) || (pos.y > worldBoxHalfSize.y + buffer) ||
            (pos.z < -buffer)                      || (pos.z > 2 * worldBoxHalfSize.z + buffer)) ? false : true;
}

function scaleAtPos(pos) {
    let posPt = worldTo2D(pos);
    let posOneOver = worldTo2D(vec3.add(pos, vec3.right()));
    return (posOneOver.x - posPt.x);
}
...
function RGBV3ToHex(color) {     // Color values are 0 to 255
    return "#" + ("00" + Math.floor(color.x).toString(16)).substr(-2) + 
                 ("00" + Math.floor(color.y).toString(16)).substr(-2) + 
                 ("00" + Math.floor(color.z).toString(16)).substr(-2);
}
function RGBToHex(r, g, b) {     // r, g, b are 0 to 255
